(function() {
  ...
  parse = function(source, code) {
    var code_text, docs_text, has_code, in_multi, language, leaving_multi, line, lines, multi_accum, save, sections, tmp_line, _i, _len;
    lines = code.split('\n');
    sections = [];
      ...
      if (line.match(language.multi_start_matcher) || in_multi) {
        ...
        if (language.name === 'coffee-script') {
          ...
          if (!in_multi) {
            ...
            leaving_multi = tmp_line.match(/###$/) != null;
            tmp_line = tmp_line.replace(/###$/, '*\/');
          } else {
            tmp_line = tmp_line.replace(/###$/, '*\/');
            leaving_multi = line.match(language.multi_end_matcher) != null;
          }
